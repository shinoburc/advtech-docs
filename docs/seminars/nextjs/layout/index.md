...
`app/_components` ディレクトリを作成し、その中にレイアウト関連コンポーネントを配置します。

今後、メニューを追加する場合はこのコンポーネントを変更してください。

まずは Menu コンポーネントを以下のように実装してください。

```tsx title="app/_components/Menu.tsx"
...
`app/_compoents/に以下のように実装してください。

```tsx title="app/_compoents/AccountInfo.tsx"
'use client';
...
```tsx title="app/_compoents/Layout.tsx"
...
Next.js では `app/layout.tsx` でアプリケーション全体のレイアウトを設定することができます。

以下のように `app/layout.tsx` を変更してください。
